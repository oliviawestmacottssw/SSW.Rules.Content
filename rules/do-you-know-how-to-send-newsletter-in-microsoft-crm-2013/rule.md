...
5. Now you have to wait while the emails send out:
    ...
    - **Bad Example - Email router** for outgoing email, then those email activities will be sent out automatically by Email router. This method is our preferred method of sending the newsletter, CRM email router can be configured to send out newsletters immediately and the user doesn't have to open Outlook while the emails are being processed. ​As per Crm tip of the day ([https://crmtipoftheday.com/979/start-planning-farewell-party-for-email-router/](https&#58;//crmtipoftheday.com/979/start-planning-farewell-party-for-email-router/)) the email router is now deprecated
