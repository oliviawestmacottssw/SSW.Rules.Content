...
Let's look at some examples and tips:
 ![](NotifyCodeChangesBad.gif) Figure: Bad example - the client cannot see any detail of what was done  ![](NotifyCodeChanges.gif) Figure: Good example - the client can see the image + the code changes highlighted in yellow 
...
**You can include the code as an attached text file.
 ![](NotePad2DiffFiles.gif) Figure: Good example - this is a Text file with a .diff extension that includes the code change from TFS. If opened using NotePad2, the client can view the code changes with green and red color (added and deleted code). 
...
For those using TFS, enable the Checkin policy and force all developers to associate every check-in with a work item.
 ![ ](SourceControl_AssociateWorkItems1.gif) ** **Figure: Make developers associate all check-ins to a work items    ![](SourceControl_AssociateWorkItems2.gif) ** **Figure: Enabling the Checkin Policy (via Project | Team Project Settings | Source Control | Add)  
