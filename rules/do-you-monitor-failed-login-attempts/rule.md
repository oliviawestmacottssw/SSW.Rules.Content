...
It is important to monitor failed login attempts to determine if you are being attacked from an external source or are having failed attempts from users within your organisation. This can be achieved with Passive Whats Up Gold Monitor.​
 ![failed-login-whatsup-gold-1.png](failed-login-whatsup-gold-1.png)Figure: This P​assive Monitor can then be applied to your Servers![failed-login-whatsup-gold-2.png](failed-login-whatsup-gold-2.png)Figure: Good example - This Passive Monitor will then record failed login attempts​It is important to also ensure that you have "Audit logon events" Group Policy applied to servers for source information on the login. 
