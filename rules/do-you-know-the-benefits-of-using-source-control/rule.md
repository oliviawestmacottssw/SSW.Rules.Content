...
<br>With the source control (we use TFS), we can share project code and cooperate with other team members. Using it allows us to track changes, compare code and even roll-back if required. Moreover, it keeps our code safe that is the most important. <br> ![](HistoryWindow.jpg) Figure: View the changes in source control on each individual file ![](HistoryCompareMenu.jpg) Figure: We can select different changesets and compare the changes ![](Compare.jpg) Figure: We can select different changesets and compare the changes. Blue = modified, Green = addition, Red = deletion 
